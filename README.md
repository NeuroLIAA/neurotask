...
After running the TMT analysis, the results are saved as a CSV file. Each row in the CSV represents a trial (valid or invalid) and contains the following columns:
...
> **Note:** For invalid trials, many of the metric columns (e.g., distances, speeds, accelerations) will be set to default values or `NaN` to indicate that the trial did not pass validation checks.
